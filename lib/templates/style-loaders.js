...
module.exports = (get, fallback = 'style-loader', plugins = () => {}) => {
	...
	const use = Object.assign.apply(Object, ['scss', 'sass', 'stylus', 'styl', 'less'].map(name => {
		const meta = {};
		const options = name === 'sass' ? {sourceMap, indentedSyntax: true} : {sourceMap};
		meta[name] = css.concat(loader(name, get(`style.${name}`), options));
		return meta;
	}).concat([{css}, {postcss}]));
