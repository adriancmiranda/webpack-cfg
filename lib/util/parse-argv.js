...
const parseOpt = (keys, key, index, argv) => {
...
const reducer = (keys, key, index, argv) => {
	if (reOPT.test(key)) {
		keys = parseOpt(keys, key, index, argv);
	} else if (index > 1) {
...
module.exports = (customArgv) => {
	const argv = as(Array, customArgv, process.argv);
	const initialValue = Object.create(null, defaultProperties(argv));
	const pairs = argv.reduce(reducer, initialValue);
	return pairs;
};
