...
module.exports = class Tasks {
	constructor(self, pattern) {
		...
		else throw new TypeError('Could not configure your webpack with this pattern');
	}
	...
	static run(parent, cwd, tasks, settings, demand) {
		const configs = [];
		Tasks[is.empty(demand) ? 'eachFile' : 'findDemandFile'](tasks, (task, cmd) => {
	...
	static load(parent, cwd, taskPath, friendlyPath) {
		try {
			...
			process.stdout.write(`${cwd}: ${error.message.replace(taskPath, friendlyPath)}\n\n${error.stack}\n`);
			process.exit(1);
		}
	}
